...
void initializeState(StateStruct& state,
  ...
  if (parameters._state_initialization == "random") {
    ...
    for (int i = 0; i < 2; i++) {
      state._spin_config.push_back(interactions._epsilon = dist(utils::rng()) * 2 - 1);       
    }
  ...
  /** @brief Example of random initialization */
    ...
    utils::handleExceptions([&]() {
      ...
      state._spin_config =  json_model_state.at("configuration").get<std::vector<int>>(); 

      /** Check that state._spin_config has exactly 2 elements */
      if (state._spin_config.size() != 2) {
        throw std::runtime_error("Invalid spin configuration: expected a vector of size 2.");
      }
...
int computeTotalEnergy(StateStruct& state,
                       ...
                       InteractionStruct& interactions) {
  int total_energy =
           parameters._alpha * interactions._epsilon * state._spin_config[0] * state._spin_config[1];
