...
void parseParameters(ModelParametersStruct &parameters, const std::string& input_folder) {
  ...
  utils::handleExceptions([&]() {
    ...
    parameters._N = json_model_params.at("N").get<int>();
    if (   parameters._N <= 0)   
    {
      throw std::runtime_error("Invalid number of nodes N: " + 
                  parameters._interactions_initialization + ". N must be larger than zero.");
    }
...
void printParameters(ModelParametersStruct &parameters){
  ...
  std::cout << "Number of spins N           : "    << parameters._N << std::endl;
