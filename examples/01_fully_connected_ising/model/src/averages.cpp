...
void updateAverages(AveragesStruct &averages,
  ...
  if (parameters._save_m) {
    ...
    for (int i = 0; i < parameters._N; i ++) {
      m += state._spin_config[i];
    }
    averages._m += m / static_cast<double>(parameters._N); 
  }
...
void saveAverages(AveragesStruct &averages,
  ...
  if (parameters._save_m) {
    utils::saveScalarAverageToFile(average_output + "magnetization.txt",
                            averages._m,
                            T,
                            mcs_av,
                           "saveMagnetization");  
...
void createFiles(ModelParametersStruct& parameters,
  ...
  if (parameters._save_m) {
    utils::createFile(average_output + "magnetization.txt", "saveMagnetization");  
  }
